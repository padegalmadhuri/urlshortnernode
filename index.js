...
const dbURL = "mongodb+srv://Madhuri:sreedhar123@cluster0-uzatp.mongodb.net/shorturl?retryWrites=true&w=majority";
...
async function authenticate(req, res, next) {
    if (req.headers.authorization == undefined) {
        ...
        jwt.verify(req.headers.authorization, 'shdoidjwqjdjoiwjiwjiwj', (err, decoded) => {
            if (err) {
                res.status(401).json({
            ...
            db.collection("users").findOne({ email: req.body.email }, (err, data) => {
                ...
                if (data) {
                    bcrypt.compare(req.body.password, data.password, function(err, result) {
                        ...
                        // result == true
                        if (result) {
                            jwt.sign({ id: data['_id'], }, 'shdoidjwqjdjoiwjiwjiwj', { expiresIn: '10h' }, function(err, token) {
                                if (err) throw err;
                                // console.log(token);
                                client.close();
                                res.status(200).json({
    ...
    require('crypto').randomBytes(32, function(ex, buf) {
        ...
        mongodb.connect(dbURL, (err, client) => {
            ...
            db.collection("users").update({ email: req.body.email }, { $set: { reset_token: token, timestamp: timestamp, expiry: expiry } }, (err, data) => {
                ...
                mailOptions.to = req.body.email;
                mailOptions.subject = ' assesment Password reset '
                mailOptions.html = `<html><body><h1>Reset Password link</h1>
